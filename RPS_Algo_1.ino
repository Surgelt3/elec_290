...
enum strategies {
  Random_Choice = 0,
  Counter_Opp_Rotation = 1,
  Beat_Last_Input = 2,
  Play_Previous_Losing = 3,
  Counter_Rotation = 4,
  Win_Stay_Lose_Shift = 5,
  Win_Replay_Lose_Shift = 6
};
...
RPSChoice beat_last_input_counter(int prev_round){
  return (RPSChoice)winningChoiceAgainst(prev_round);
}
...
RPSChoice counter_opp_rotation(int prev_round){
  ...
  RPSChoice rotation = abs(prev_round_results_2 - prev_round_results);

  RPSChoice rotation_counter = opponentChoices[prev_round]+rotation+1;
...
strategies guess_strategy(){
  ...
  if (win_replay_lose_shift(roundsPlayed-2) == opponentChoices[roundsPlayed-1]){
    ...
    counter++;
    list_strats[counter] = Counter_Opp_Rotation;
    counter ++; 
  }

  strategies choice = list_strats[random(0,counter)];
  return choice;
}
